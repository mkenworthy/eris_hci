...
# the spider. A grid is created in this direction with zero point on the outer diameter. The spider in this coordinate system is
...
M2_ERIS = 646.5*2. #DSM wind screen external diameter (3)
...
blurring_out_of_pupil_plane = 125 *um #micron as decided by Matthew Kenworthy and David Doelman 12-01-2018
blurring_flexing_motion = 300 * um #300 micron absolute worst case, could be 150 and would be OK as decided by Matthew Kenworthy and David Doelman 12-01-2018
...
print('The physical size of the ERIS entrance pupil diameter = {0} mm'.format(np.round(M1_ERIS/Pivot_point_spiders*nx*pixelsize/mm,decimals = 3)))
...
box = add_box(pupil_grid,(-outer_diameter_M3_stow/2./Diameter_array,0), (outer_diameter_M3_stow/Diameter_array,M2_ERIS/Diameter_array))
...
FitsOut.writeto('VLT_ERIS_pupil_A_entrance_pupil.fits', clobber = True)

FitsOut = pf.HDUList()
FitsOut.append(pf.ImageHDU(1.*VLT_ERIS_blurred_1.reshape((nx,nx))))
FitsOut.writeto('VLT_ERIS_pupil_B_undersized_out_of_pupil_plane.fits', clobber = True)

FitsOut = pf.HDUList()
FitsOut.append(pf.ImageHDU(1.*VLT_ERIS_blurred_2.reshape((nx,nx))))
FitsOut.writeto('VLT_ERIS_pupil_C_tightly_undersized_all_uncertainties.fits', clobber = True)
...
plt.title('Pupil C, conservatively undersized pupil ERIS (all uncertainties)')
plt.savefig('VLT_ERIS_pupil_C_conservatively_undersized_all_uncertainties.pdf')
plt.show()
